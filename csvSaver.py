    ...
    def new_data(self, s, ts, d):
        ...
        if s == 'eeg':
            ...
            for i in range(n):
                ...
                self.data_eeg[self._eeg_cnt, 0] = ts[i]
                self.data_eeg[self._eeg_cnt, 1:] = d[i]
                self._eeg_cnt += 1
        elif s == 'acc':
            for i in range(n):
                if self._acc_cnt >= self.data_acc.shape[0]:
                    self.data_acc = self._grow(self.data_acc, self._acc_cnt, self.data_acc.shape[0] * 2)
                self.data_acc[self._acc_cnt, 0] = ts[i]
                self.data_acc[self._acc_cnt, 1:] = d[i]
                self._acc_cnt += 1
        elif s == 'mar':
            for i in range(n):
                ...
                self.data_marker[self._mar_cnt, 0] = ts[i]
                # 提取标量值，避免存储 numpy 数组（否则 str(arr) → '[0]' 而非 '0'）
                label = d[i]
                if isinstance(label, np.ndarray) and label.size == 1:
                    label = label.item()
                self.data_marker[self._mar_cnt, 1] = label
                self._mar_cnt += 1

    def flush_data(self):
        if self.data_eeg is None:
        ...
        with open(csv_path, 'w', newline='', encoding='utf-8-sig') as f:
            ...
            # Marker 对齐：每个 marker 独占一行，碰撞时寻找最近的空行
            markers = [''] * eeg_ts.shape[0]
            if mar_data.shape[0] > 0:
                marker_ts = mar_data[:, 0]
                marker_labels = mar_data[:, 1]
                # 过滤超出 EEG 数据时间范围的无效 marker
                max_eeg = eeg_ts[-1]
                valid_mask = marker_ts <= max_eeg
                n_dropped = (~valid_mask).sum()
                if n_dropped > 0:
                    print(f'[CSV] 丢弃 {n_dropped} 个超出 EEG 时间范围的 marker (max_eeg={max_eeg:.3f}s)')
                marker_ts = marker_ts[valid_mask]
                marker_labels = marker_labels[valid_mask]
                used = set()
                for m_ts, m_label in zip(marker_ts, marker_labels):
                    base = int(np.argmin(np.abs(eeg_ts - m_ts)))
                    # 碰撞时双向搜索最近的空行（最多偏移 ±5 行）
                    idx = base
                    offset = 0
                    while offset <= 5:
                        for candidate in (base + offset, base - offset):
                            if 0 <= candidate < len(markers) and candidate not in used:
                                idx = candidate
                                break
                        if idx not in used:
                            break
