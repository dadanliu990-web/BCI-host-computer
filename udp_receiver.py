...
from lsl_config import (
    ...
    UDP_EEG_CHANNELS, UDP_SEQ_BYTES, UDP_POLL_INTERVAL_MS,
    LSL_EEG_NAME,
)
...
class UdpReceiver(QObject):
    ...
    evt_udp_data = pyqtSignal(str, np.ndarray, np.ndarray, int)
    # 参数: stream_name (str), timestamps (1D ndarray), data (2D ndarray), seq_num (int)
    ...
    def _poll(self):
        ...
        while True:
            ...
            eeg_raw = data[1:-(1 + UDP_SEQ_BYTES)]  # 256 字节 EEG 数据
            seq_raw = data[-(1 + UDP_SEQ_BYTES):-1]  # 4 字节序列号
            try:
                values = struct.unpack(f'<{UDP_EEG_CHANNELS}i', eeg_raw)
                seq_num = struct.unpack('>I', seq_raw)[0]
            except struct.error:
                continue
            ...
            self.evt_udp_data.emit(LSL_EEG_NAME, ts, arr, seq_num)
